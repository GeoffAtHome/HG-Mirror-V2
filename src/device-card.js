...
class DeviceCard extends PolymerElement {
    static get template() {
        ...
        <template is="dom-if" if="{{radiator}}">
            <radiator-device on="[[on]]" node_id="[[node.addr]]" data="[[data]]"
             lastseen="[[node.childValues.lastComms.val]]"></radiator-device>
        </template>
        <template is="dom-if" if="{{sensor}}">
            <sensor-device node_id="[[node.addr]]" data="[[data]]"
             lastseen="[[node.childValues.lastComms.val]]"></sensor-device>
        </template>
`;
    }
